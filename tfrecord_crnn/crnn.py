...
class CRNN(object):
    ...
    def construct_graph(self):
        ...
        # decoded:decoded[0]->decoded[0]._indices;decoded[0]->_values;decoded[0]->_dense_shape
        # log_prob: 序列的概率
        return net_output, decoded, max_char_count
        # net_output(97,32,134) decoded: max_char_count:97
    ...
    def _cnn(self, inputs):
        ...
        # 这里生成的是（32,2,97,512）的特征图，列不为1需要修改网络结构

        ### add by shangtzh
        conv9 = self._conv2d(inputs=pool4, filters=512, padding="same", batch_norm=True, name='conv9')
        # (32,2,97,512)
        conv10 = self._conv2d(inputs=conv9, filters=512, padding="same", batch_norm=True, name='conv10')
        pool5 = tf.layers.max_pooling2d(inputs=conv10, pool_size=[2, 1], strides=[2, 1], padding="valid")
        # (32,1,97,512)
        # features = tf.squeeze(pool4, axis=1, name='features')
        # 去掉维度为1的维度
        features = tf.squeeze(pool5, name='features')
