...
padding_lenth = 24
null_code = 133
img_save_fold = 'resized_image_test'
...
def main(argv):
    if(len(argv)<3):
        print("please input args(image_path,train/test/val)")
        return
    image_path = argv[1]
    # dict_path = argv[2]
    # data_for = argv[3]
    ...
    # if not os.path.exists(os.path.join('tfrecord',data_for)):
    #     os.makedirs(os.path.join('tfrecord',data_for))
    # tfrecord_writer  = tf.python_io.TFRecordWriter(path='tfrecord/'+data_for+'/'+fileName)
    for j in range(num_instance):
        print(j)
        print(addrs_image[-j-1])
